...
public class ListImporter {
     ...
     * contig   id                      geno    smp1    smp1    smp2    smp2    smp3    smp3    smp4   ...
    ...
    public void read(File listFile, Set<String> samples) throws FileNotFoundException, IOException {
        if (verbose) System.err.println("Loading nodes from LIST file...");
	...
	while ((listLine=listReader.readLine())!=null) {
		...
		// add a new node
		nodeId++;
		n = new Node(nodeId, id, contig, 0, 0, genotype, 0.0); // AF will be updated later
		nodes.add(n);
	...
	for (Node n : nodeSamplesMap.keySet()) {
            ...
            n.af = (double)nodeSamples.size() / (double)sampleNodesMap.size();
        }
	if (verbose) {
	    System.err.println("");
